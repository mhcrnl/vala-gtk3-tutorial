Entry
=====
The Entry widget is used to accept short amounts of text from the user, including the handling of passwords if required. A typical use case would be to receive the name of the user.

===========
Constructor
===========
Construction of the Entry widget is made using::

  var entry = new Gtk.Entry();

An associated :doc:`entrybuffer` can be specified at construction for use with the Entry if required::

  var entry = new Gtk.Entry.with_buffer(entrybuffer);

In most cases an EntryBuffer will not be required as one is automatically created with the Entry widget.

=======
Methods
=======
The set text within an Entry can be obtained via::
...
.. literalinclude:: _examples/entry.vala

Download: :download:`Entry <_examples/entry.vala>`
